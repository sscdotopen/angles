...
//  val fields = MultiFields.getFields(reader)
  ...
  docIds foreach { similarityGraph.addVertex(_) }
  ...
  scala.io.Source.fromFile(new File(dataDir, "communities-manual.tsv")).getLines foreach { line =>
    if (!line.startsWith("#")) {
      if (line == "") {
        community += 1
      } else {
        explorerToCommunity.put(line, community)
      }
    }
  }
  ...
  components foreach { vertices =>
    ...
    val newsItems = vertices map { docId =>
      ...
      val community = explorerToCommunity(reader.document(docId).getField("screenname").stringValue())
    ...
    //if (communities.size > 1) {
      ...
      for (community <- communities) {
        ...
        val newsItemsToDisplay = newsItems.toSeq.filter { _.community == community }
                                                .sortBy(_.stddevs)
                                                .reverse
